...
parser.add_argument("--max-length", help="The maximum length of the program body", type=int, default=1)
...
parser.add_argument("destination", help="The directory that will contain the dataset", type=str)
...
class Tqdm:
    ...
    def on_finish_enumeration(self, n_programs: int):
        ...
        self._tqdm_dump_dataset = tqdm(total=n_programs, desc="Dataset Generation")
    def on_dump_dataset(self, n: int):
        self._tqdm_dump_dataset.update(n)
    def finish(self):
        self._tqdm_dump_dataset.close()
tqdm_for_generation = Tqdm()
callback = ProgressCallback(lambda p: tqdm_for_generation.on_generate_program(p), lambda x: tqdm_for_generation.on_finish_enumeration(x), lambda x: tqdm_for_generation.on_dump_dataset(x))
...
generate_dataset(LINQ,
                 DatasetSpec(args.value_range, args.max_list_length, args.num_examples, args.min_length, args.max_length),
                 EquivalenceCheckingSpec(0, args.num_examples_for_pruning, np.random.RandomState(root_rng.randint(SEED_MAX))),
